...
class DecoderEvent(nn.Module):
    ...
    def forward(self, X, attention):
        ...
        attn_applied = torch.bmm(attention.unsqueeze(1), X).squeeze(1) # attention.shape[128,10], X.shape [128,10,67], attention.unsqueeze(1).shape[128,1,10]
        # attn_applied = self.dropout(attn_applied)
        #pdb.set_trace()
        # Compute prediction based on latent dimension
        output = self.hidden(attn_applied).relu()  # output.shape[128,67]
        # output = self.dropout(output)
        output = self.out(output)
        # Apply softmax for distribution
        output = F.log_softmax(output, dim=1) # output.shape[128,67]
